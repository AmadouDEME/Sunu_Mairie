...
import { Search, X, Mail, Phone, MapPin, Calendar, CreditCard } from "lucide-react";
...
const CITIZENS_DATA = [
  {
    ...
    status: "Actif",
    history: [
      { id: "DM-2026-001", type: "Acte de naissance", status: "En attente", date: "19/06/2026" },
      { id: "DM-2025-104", type: "Signalement", status: "Prêt", date: "15/12/2025" },
      { id: "DM-2025-089", type: "Certificat de mariage", status: "Prêt", date: "02/10/2025" },
    ],
  },
  {
    id: "CIT-002",
    ...
    status: "Actif",
    history: [
      { id: "DM-2026-002", type: "Occupation", status: "Approuvé", date: "18/06/2026" },
      { id: "DM-2026-015", type: "Signalement", status: "Prêt", date: "12/03/2026" },
    ],
  },
  {
    id: "CIT-003",
    ...
    status: "Actif",
    history: [
      { id: "DM-2026-003", type: "Certificat de mariage", status: "Prêt", date: "17/06/2026" },
    ],
  },
  {
    id: "CIT-004",
    ...
    status: "Actif",
    history: [
      { id: "DM-2026-004", type: "Signalement", status: "Rejeté", date: "16/06/2026" },
      { id: "DM-2026-020", type: "Acte de naissance", status: "Prêt", date: "24/02/2026" },
    ],
  },
  {
    id: "CIT-005",
    ...
    status: "Actif",
    history: [
      { id: "DM-2026-005", type: "Acte de naissance", status: "En attente", date: "15/06/2026" },
      { id: "DM-2026-033", type: "Signalement", status: "Prêt", date: "05/05/2026" },
    ],
  },
  {
    id: "CIT-006",
    ...
    status: "Actif",
    history: [
      { id: "DM-2026-006", type: "Certificat de décès", status: "Prêt", date: "14/06/2026" },
    ],
  },
];
...
export default function CitoyensPage() {
  ...
  const filteredCitizens = useMemo(() => {
    return CITIZENS_DATA.filter((citoyen) => {
      ...
      const matchesRegion =
        regionFilter === "all" || citoyen.region.toLowerCase() === regionFilter.toLowerCase();
    ...
    <div className={styles.container}>
      ...
      <div className={styles.titleSection}>
        ...
        <p className={styles.subtitle}>
          Consultez l'annuaire des citoyens inscrits sur l'application mobile et l'historique de leurs démarches.
        </p>
      </div>
      ...
      <div className={styles.mainLayout}>
        <div className={`${selectedCitizen ? styles.listCol : styles.listColFull}`}>
            ...
            <table className={styles.table}>
              <thead>
                <tr>
                  ...
                  <th className={styles.th} style={{ textAlign: "center" }}>Démarches</th>
                ...
                {filteredCitizens.length > 0 ? (
                  filteredCitizens.map((item) => (
                    ...
                    >
                      ...
                      <td className={styles.td} style={{ fontFamily: "monospace" }}>{item.nin}</td>
                      ...
                      <td className={styles.td} style={{ textAlign: "right", fontWeight: 700, color: "var(--success-text)" }}>
                        {item.spent} F
                      </td>
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan={6} className={styles.td} style={{ textAlign: "center", padding: "40px 0", color: "var(--text-muted)" }}>
        ...
        {selectedCitizen && (
          <div className={styles.detailCol}>
            <div className={`${styles.detailCard} glassmorphism`}>
              <button onClick={() => setSelectedId(null)} className={styles.closeBtn} aria-label="Fermer">
                <X size={16} />
              </button>

              {/* Profile Header */}
              <div className={styles.profileHeader}>
                <div className={styles.avatar}>{getInitials(selectedCitizen.name)}</div>
                <div className={styles.profileTitles}>
                  <h3 className={styles.name}>{selectedCitizen.name}</h3>
                  <span className={styles.nin}>NIN : {selectedCitizen.nin}</span>
                </div>
              </div>
              ...
              <div className={styles.statsRow}>
                <div className={styles.statBlock}>
                  <span className={styles.statVal}>{selectedCitizen.requestsCount}</span>
                  <span className={styles.statLabel}>Demandes</span>
                </div>
                <div style={{ width: 1, backgroundColor: "var(--border)" }} />
                <div className={styles.statBlock}>
                  <span className={styles.statVal} style={{ color: "var(--success-text)" }}>
              ...
              <div className={styles.infoSection}>
                ...
                <div className={styles.infoRow}>
                  <span className={styles.infoLabel}>
                    <Mail size={14} style={{ marginRight: 6, display: "inline", verticalAlign: "middle" }} />
                    Email
                  </span>
                  <span className={styles.infoVal}>{selectedCitizen.email}</span>
                </div>
                <div className={styles.infoRow}>
                  <span className={styles.infoLabel}>
                    <Phone size={14} style={{ marginRight: 6, display: "inline", verticalAlign: "middle" }} />
                    Téléphone
                  </span>
                  <span className={styles.infoVal}>{selectedCitizen.phone}</span>
                </div>
                <div className={styles.infoRow}>
                  <span className={styles.infoLabel}>
                    <MapPin size={14} style={{ marginRight: 6, display: "inline", verticalAlign: "middle" }} />
                    Adresse
                  </span>
                  <span className={styles.infoVal}>{selectedCitizen.address}</span>
                </div>
                <div className={styles.infoRow}>
                  <span className={styles.infoLabel}>
                  ...
                  <span className={styles.infoVal}>{selectedCitizen.registeredDate}</span>
                </div>
              </div>
                  ...
                  {selectedCitizen.history.map((hist) => (
                    <div key={hist.id} className={styles.historyItem}>
                      <div>
                        ...
                        <div className={styles.histDate}>Réf: {hist.id} • le {hist.date}</div>
                      </div>
                      <span
                        style={{
                          fontSize: "0.7rem",
